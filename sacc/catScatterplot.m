function [] = catScatterplot(X, varargin)
% function [] = categoricalscatterplot()
%
% Based on 'categoricalscatterplot.m' from Dinesh Natesan. Modified and
% extended by Patrick Jendritza 2022
%
...
addParameter(p, 'spreadWidth', 0.75, @(x) isnumeric(x));
addParameter(p, 'boxWidth', 0.75, @(x) isnumeric(x));
% Plotting styles
...
% Plotting order
addParameter(p, 'plotOrder', [], @(x) isnumeric(x));
% X-axis centers for plotting
addParameter(p, 'xcenters', [], @(x) isnumeric(x));
...
plotOrder = parsed.plotOrder;
if isempty(plotOrder)
   plotOrder  = unique(Group);
end

xcenters = parsed.xcenters;
if isempty(xcenters)
   xcenters  = unique(Group);
end

% swap group orders according to vector
b = Group'==unique(Group);
for i = 1:size(b,1)
 thisMult = i;
 thisLoc = plotOrder(i);
 bb(thisLoc,:) = b(thisLoc,:)*thisMult;   
end
Group = sum(bb)';


parsed.Labels = parsed.Labels(plotOrder);
...
% center around new x value
for i = 1:length(groups)
new_data{i,2} = new_data{i,2}-i+xcenters(i);
end
...
for i = 1:length(groups)
    ...
    ip = xcenters(i); % xcenter for plotting
    ...
    % Draw box
    patch([ip-boxWidth/2, ip-boxWidth/2, ip+boxWidth/2, ip+boxWidth/2]',...
    ...
    scatter(new_data{i,2}(plot_vars{i,2}), new_data{i,1}(plot_vars{i,2}),...
        parsed.MarkerSize, plot_vars{i,1}(plot_vars{i,2},:), 'filled', 'MarkerFaceAlpha',.5);
    scatter(new_data{i,2}(~plot_vars{i,2}), new_data{i,1}(~plot_vars{i,2}),...
        parsed.MarkerSize, plot_vars{i,1}(~plot_vars{i,2},:));
    
%     disp(num2str(size(new_data{i,2}(plot_vars{i,2})))) % PJ debug
    
    % Draw median
    plot([ip-boxWidth/2, ip+boxWidth/2], ...
...
%         'Marker','none');
    ...
    if any(data{i,1}>whisker(2))
        % Outlier present
        plot([ip-boxWidth/5, ip+boxWidth/5], ...
            [whisker(2), whisker(2)], ...
            'LineStyle', '-', ...
        ...
        if parsed.WhiskerLine
            % Draw top whiskers
            plot([ip, ip], [whisker(2), imp_quantiles(3)],...
        ...
        if parsed.WhiskerLine
            % Draw top whiskers
            plot([ip, ip], [new_data{i,1}(closest_point), imp_quantiles(3)],...
                'LineStyle', parsed.WhiskerLineStyle, ...
                'Color', parsed.WhiskerColor,...
                'LineWidth', parsed.WhiskerLineWidth,...
                'Marker','none');
    ...
    if any(data{i,1}<whisker(1))
        ...
        if parsed.WhiskerLine
            % Draw top whiskers
            plot([ip, ip], [whisker(1), imp_quantiles(3)],...
                'LineStyle', parsed.WhiskerLineStyle, ...
                'Color', parsed.WhiskerColor,...
                'LineWidth', parsed.WhiskerLineWidth,...
                'Marker','none');
        ...
        if parsed.WhiskerLine
            ...
            plot([ip, ip], [new_data{i,1}(closest_point), imp_quantiles(1)],...
                'LineStyle', parsed.WhiskerLineStyle, ...
                'Color', parsed.WhiskerColor,...
                'LineWidth', parsed.WhiskerLineWidth,...
                'Marker','none');
