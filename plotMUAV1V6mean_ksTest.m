function [stats] = plotMUAV1V6mean_ksTest( fullSessionName, aMU1eS, aMU6eS, preT, postT, xlimPl, Fs, msSmooth)
...
timeXa = linspace(-preT,postT,size(aMU1eS,2)); % create time vector
...
signstart =   0.0;
signsend  =   0.65;
stX = xlimPl(1)*1.08; % x position for the plotting the asterisk
...
nConds = 1; % only one condition for now
...
for ishank = 1:2 % loop through the V1 shanks
    ...
    for icond = 1:nConds
        ...
        MUAaTrPoolMeanV1{ishank,icond} = mean(reshape(permute(tmp(:,:,mod_V1 & shankChns),[2,1,3]),size(aMU1eS,2),[],1),2);
        % calculate SEM across all pooled responsive channels and trials
        MUAaTrPoolStdV1{ishank,icond} = sem(reshape(permute(tmp(:,:,mod_V1 & shankChns),[2,1,3]),size(aMU1eS,2),[],1)')';
        ...
        for ich=find(mod_V1 & shankChns)
            ...
            MUAaTrChSEMV1{ishank,icond,ich} = sem(reshape(permute(tmp(:,:,ichBin & shankChns),[2,1,3]),size(aMU1eS,2),[],1)')';
        end
...
for ishank = 1:4  % loop through the V6 shanks
    ...
    for icond = 1:nConds
        MUAaTrPoolMeanV6{ishank,icond} = mean(reshape(permute(tmp(:,:,mod_V6 & shankChns),[2,1,3]),size(aMU6eS,2),[],1),2);
        MUAaTrPoolStdV6{ishank,icond} = sem(reshape(permute(tmp(:,:,mod_V6 & shankChns),[2,1,3]),size(aMU6eS,2),[],1)')';
        %calculate SEM for each responsive channel independenty
        for ich=find(mod_V6 & shankChns)
            ichBin = false(size(mod_V6)); ichBin(ich) = true;
            MUAaTrChSEMV6{ishank,icond,ich} = sem(reshape(permute(tmp(:,:,ichBin & shankChns),[2,1,3]),size(aMU6eS,2),[],1)')';
        end
        % calculate mean SED across responsive channels
        try
...
disp([num2str(numel(find(mod_V1(pmap)))) ' of 32 modulated channels on shank V1_1A'])
...
try
    shadedErrorBar(timeXa,gaussSmooth( MUAaTrPoolMeanV1{1,1}', Fs, msSmooth), gaussSmooth( MUAaTrMeanSEMV1{1,1}'*xSEM, Fs, msSmooth),'lineprops',{'color', 'k','linewidth',1},'transparent',1);
catch
end
axis tight
xlim(xlimPl)
...
disp([num2str(numel(find(mod_V1(pmap)))) ' of 32 modulated channels on shank V1_1B'])

subplot(4,4,14)
timeXa = linspace(-preT,postT,size(MUAaTrAvrgV1,1));

try
    shadedErrorBar(timeXa,gaussSmooth( MUAaTrPoolMeanV1{2,1}', Fs, msSmooth), gaussSmooth( MUAaTrMeanSEMV1{2,1}'*xSEM, Fs, msSmooth),'lineprops',{'color', 'k','linewidth',1},'transparent',1);
catch
end
axis tight
xlim(xlimPl)
...
imagesc(timeXa, 1:32, MUAaTrAvrgV6(:,pmap)');
colormap hot
hold on
plot([0 0], [1 32], '--w' )
xlabel('Time (s)')
...
disp([num2str(numel(find(mod_V6(pmap)))) ' of 32 modulated channels on shank V6_1A'])
box off
spPos = get(gca, 'Position');  % [left, bottom, width, height]
cb = colorbar;
cb.Position = [0.01+spPos(1)+spPos(3) spPos(2) 0.007 spPos(4)*0.95];
...
try
    shadedErrorBar(timeXa,gaussSmooth( MUAaTrPoolMeanV6{1,1}', Fs, msSmooth), gaussSmooth( MUAaTrMeanSEMV6{1,1}'*xSEM, Fs, msSmooth),'lineprops',{'color', 'k','linewidth',1},'transparent',1);
catch
end
axis tight
xlim(xlimPl)
...
disp([num2str(numel(find(mod_V6(pmap)))) ' of 32 modulated channels on shank V6_1B'])
box off
spPos = get(gca, 'Position');  % [left, bottom, width, height]
cb = colorbar;
cb.Position = [0.01+spPos(1)+spPos(3) spPos(2) 0.007 spPos(4)*0.95];
...
try
    shadedErrorBar(timeXa,gaussSmooth( MUAaTrPoolMeanV6{2,1}', Fs, msSmooth), gaussSmooth( MUAaTrMeanSEMV6{2,1}'*xSEM, Fs, msSmooth),'lineprops',{'color', 'k','linewidth',1},'transparent',1);
catch
end
axis tight
xlim(xlimPl)
...
disp([num2str(numel(find(mod_V6(pmap)))) ' of 32 modulated channels on shank V6_2B'])
box off
spPos = get(gca, 'Position');  % [left, bottom, width, height]
cb = colorbar;
cb.Position = [0.01+spPos(1)+spPos(3) spPos(2) 0.007 spPos(4)*0.95];
...
try
    shadedErrorBar(timeXa,gaussSmooth( MUAaTrPoolMeanV6{4,1}', Fs, msSmooth), gaussSmooth( MUAaTrMeanSEMV6{4,1}'*xSEM, Fs, msSmooth),'lineprops',{'color', 'k','linewidth',1},'transparent',1);
catch
end
axis tight
xlim(xlimPl)
...
disp([num2str(numel(find(mod_V6(pmap)))) ' of 32 modulated channels on shank V6_2A'])
box off
spPos = get(gca, 'Position');  % [left, bottom, width, height]
cb = colorbar;
cb.Position = [0.01+spPos(1)+spPos(3) spPos(2) 0.007 spPos(4)*0.95];
...
try
    shadedErrorBar(timeXa,gaussSmooth( MUAaTrPoolMeanV6{3,1}', Fs, msSmooth), gaussSmooth( MUAaTrMeanSEMV6{3,1}'*xSEM, Fs, msSmooth),'lineprops',{'color', 'k','linewidth',1},'transparent',1);
catch
end
axis tight
xlim(xlimPl)
